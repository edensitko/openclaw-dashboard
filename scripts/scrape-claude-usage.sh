...
if command -v claude &>/dev/null; then
  ...
  # macOS Claude Desktop installs here
  CLAUDE_BIN=$(find "$HOME/Library/Application Support/Claude" -name "claude" -type f 2>/dev/null | sort -V | tail -1)
  if [ -z "$CLAUDE_BIN" ]; then
    echo "Error: claude CLI not found in PATH or Application Support"
    exit 1
  fi
fi
...
if [ "$CLAUDE_READY" = false ]; then
  ...
  tmux send-keys -t "$SESSION" "cd ${WORKSPACE_DIR} && \"${CLAUDE_BIN}\"" Enter
